...
int main() {
	int n = 3, m = 3, N = 20000, l;
	double a = 1, b = 2, c = 0, d = 1, h, k, **matriz, *x, *y;
	double lambda, sigma, z, NORM, TOL = 0.0001;
	FILE *fp = fopen("solucao.txt", "w");
	FILE *fq = fopen("matriz_1.txt", "w");
	...
	while(l<=N) {
		...
		for(int j=m-2; j>=2; j--) {
			...
			if(NORM <= TOL) {
				...
				// Assim como imprimirei a matriz em uma arquivo para plotar a imagem
